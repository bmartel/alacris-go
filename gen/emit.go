...
type Options struct {
	...
	// PackageDoc, when set, replaces the generated package comment. The
	// shipped Alacris UI wrappers use it so the comment describes the design
	// system rather than "the project's components".
	PackageDoc string
}
...
func emitShared(m *Manifest, opts Options) ([]byte, error) {
	...
	if pkgDoc == "" {
		pkgDoc = fmt.Sprintf(`Package %s renders the project's alacris components from Go.
...
produce is the element, its props and its light-DOM children.`, opts.Package)
	...
	w.doc(fmt.Sprintf(`Tags lists every component in this package.

It is what alacris.Pending wants, so that elements do not flash before their
module has loaded:

	@alacris.Pending{Tags: %s.Tags}`, opts.Package))
