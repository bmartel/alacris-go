...
description: Turn your define() calls into typed Go — props structs, event details, slot names and theming contracts — with one source of truth.
sidebar:
  order: 3
---
...
```go title="internal/ui/components_gen.go"
...
When a component genuinely builds its props at runtime, describe it by hand:
...
3. **Generate from it**, alone or mixed with sources:
   ...
   ```bash
   alacris-go generate ./alacris.components.json ./web/other -o ./ui
   ```
...
```bash
alacris-go generate ./web -o ./ui -strip ala-
