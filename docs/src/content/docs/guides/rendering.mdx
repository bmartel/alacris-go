...
and a `false` one disappears. That is the opposite of how props behave, and it
is deliberate: `hidden` and `disabled` are HTML, not component API.
...
visible to a crawler — which makes it the right home for content that matters
when JavaScript has not arrived yet.
...
`ui.Tags` is generated, so the list cannot fall behind the components. The
declaration defaults to `visibility: hidden`; pass `Style` to choose another.
