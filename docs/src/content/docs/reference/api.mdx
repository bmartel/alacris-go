---
title: Go API
description: Every exported type and function across the three packages, with the one-line reason each exists.
sidebar:
  order: 1
---
...
| `AssetCore`, `AssetCoreDev`, `AssetStore`, `AssetContext`, `AssetSignal`, `AssetLive` | Served file names. |

### Page setup

| | |
| --- | --- |
| `Config` | Base, Dev, Modules, Imports, Live, Session, Endpoint, Nonce, Version. |
| `Scripts(Config) templ.Component` | The import map, your modules, and the live client. Goes in `<head>`. |
...
| `(*Server) NewSession() *Session` | One per page render. |
...
| | |
| --- | --- |
| `(*Session) ID() string` | The capability to put in the page. |
...
| `Options` | TTL, Buffer, MaxDetail, Heartbeat, AllowOrigin, Logger, Now. |
